...
function dashOut(arr: string[]) {
  ...
  if (count < arr.length && target instanceof Element) {
    ...
    $("#self-introduction").html(
      `I'm a <span class = "key-message">front-end developer</span>, with <span class = "key-message">digital marketing</span> and <span class = "key-message">interdisciplinary translation</span> background.`
    );
  }
}
